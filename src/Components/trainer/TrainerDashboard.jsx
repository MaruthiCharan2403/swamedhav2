...
const TrainerDashboard = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await axios.get('/api/trainer/getlevels', {
          headers: { Authorization: `${sessionStorage.getItem('token')}` },
        });
        // The backend gives an array of courses directly, update state accordingly
        setAssignedCourses(response.data); // <<-- CHANGED: no .assignedCourses
        setLoading(false);
      } catch (err) {
                  ...
                  <div className="mb-6">
                    <div className="space-y-2">
                      {(course.terms || []).map((term) => (
                        <div 
                          key={term.termId}
                          onClick={() => navigate(`/trainer/level?id=${course._id}&term=${term.termId}`)}
